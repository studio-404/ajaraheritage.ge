...
function club_menu()
{
	...
	$out = '';
    $sql = "SELECT id, title, idx, redirectlink, menutitle, level, menuid, category FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND menuid = 44 AND deleted = 0 AND masterid = 0 AND visibility = 1 ORDER BY position asc;";
...
//		menu-selected
		...
		$out .= '
						<li'.(($sections[$idx]["id"] == $active && $sections[$idx]["id"] != 1) ? ' class="active"':'').'>
				'."\n";
...
function news()
{
    $sql = "SELECT * FROM " . c("table.pages") . " WHERE language = '" . l() . "' AND visibility = 1 AND menuid = 2 AND `deleted` = '0' ORDER BY postdate DESC LIMIT 4;";
    ...
    foreach ($news AS $item)
    {
        ...
        $link = href($item['id']);
	        ...
	        $out .= '
				<div class="col-sm-3">
					<a href="'.$link.'" class="NewsItem">
						<div class="Image">
							<img src="crop.php?img='.$item['image1'].'&n='.(($count % 2 != 0) ? '5' : '7').'"/>
						</div>
						<div class="Date"><span>' . convert_date($item["postdate"]) . '</span></div>
						<div class="Title">
							' . $item['title'] . '
							<div class="ReadmoRe"><span>'.l('readmore').'</span></div>
						</div>
					</a>
				</div>
		        ';
...
{
    $sql = "SELECT * FROM ".c("table.catalogs")." WHERE menuid='".$id."' AND  language = '" . l() . "' AND visibility = 1 and deleted=0 order by position";
    $homepage = db_fetch_all($sql);
    if (empty($homepage))
        return NULL;
    $out = NULL;
				...
				$link = href($child["id"], array(), l(), $home['id']);
					...
					if($home['id']==$pid){
					$out .= '
					<li class="active"><a href="'.$link.'">'.$home['title'].'</a></li>
		               		'."\n";
					}else{
					$out .= '
